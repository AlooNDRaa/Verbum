...
function Mensajes(props: MensajesProps) {
  ...
  const displayedChats = chats.filter(
    (chat) => chat.from === selectedUser || chat.from === currentUser
  );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    Socket.emit("chat", newChat, (confirmation: boolean) => {
      if (confirmation) {
  ...
  const receiveChat = useCallback(
    (newChat: { body: string; from: string; receptor: string }) => {
      ...
      setChats((state) => [...state, updatedChat]);
    },
    [selectedUser]
  );
    ...
    <>
      ...
      <div className="w-full h-screen bg-[#161616] opacity-90 flex items-center justify-center pb-[48px]">
        <form
          ...
          className="absolute bottom-0 flex items-stretch w-fit p-2 "
        >
          <input
            type="text"
        ...
        <ul className="overflow-y-scroll w-[70rem]  h-[40rem]">
          {displayedChats.map((chat, i) => (
            <li
              className={`text-white text-1xl my-2 p-2 table rounded-md ${
                chat.from === currentUser ? "bg-[#C83C83] ml-[40vw]" : `bg-[#f472b6] mr[5vw]`
              }`}
              key={i}
            >
